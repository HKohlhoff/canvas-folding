...
const PLUGIN_ID = "plugin-template";

const banner = `/*
THIS IS A GENERATED/BUNDLED FILE BY ESBUILD.
For the source code, see the plugin repository.
*/`;
...
console.log(
  `👀 Watch mode active${
    isProd ? " (production)" : ""
  } — building to release/${deployMode ? " and deploying to the vault plugin folder." : "."}`
);
...
process.on("SIGINT", async () => {
  for (const watcher of staticWatchers) {
    try {
      ...
      // Continue closing the remaining watchers during shutdown.
    }
  }
  await ctx.dispose();
  console.log("\n🛑 Watch mode stopped.");
