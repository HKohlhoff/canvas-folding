...
Die Branch-Controls sind per Tastatur erreichbar und folgen ihrer räumlichen Position innerhalb einer Zeile von links nach rechts, danach zeilenweise von oben nach unten. `Enter` oder `Leertaste` klappt den Zweig ein beziehungsweise aus; `Umschalt+F10`, die Kontextmenütaste oder auf dem Mac `Wahltaste+Enter` öffnet die Ebenenauswahl. Der Griff der Canvas-Toolbar lässt sich außer durch Ziehen auch mit den Pfeiltasten bewegen. Nach einer Tastaturaktion bleibt der Fokus auf der ausgelösten Funktion.
