...
Canvas Tree merkt sich Collapse-State, Sichttiefe und temporäre Freigaben immer für die laufende Plugin-Sitzung. So wird der letzte Sitzungszustand wiederhergestellt, wenn man in einem Tab zu anderen Dateien und anschließend zum Canvas zurücknavigiert. `Remember canvas states between sessions` speichert diesen Zustand zusätzlich nach Canvas-Pfad in der Plugin-Datei `data.json`, sodass er auch nach einem Neustart von Obsidian oder des Plugins verfügbar ist. Einträge gelöschter Canvas-Dateien werden automatisch bereinigt; unbekannte Node-IDs bleiben erhalten und werden ignoriert, damit ein noch nicht vollständig aufgebauter Canvas keinen Zustand irrtümlich löscht. Die Verwaltungsaktionen in den Settings betreffen nur den dauerhaften Speicher; aktuelle Sitzungszustände bleiben unverändert. Die `.canvas`-Datei wird in beiden Fällen nicht verändert.
